...
const StockProducts = ({ addToBuyListHandler }) => (
    ...
    <div className="flex border border-gray-200 p-2 rounded mt-2 overflow-x-auto">
      {StockProductsData.map((stockProductData) => (
        <article
          ...
          onClick={addToBuyListHandler(stockProductData.itemNumber)}
        >
          ...
          <button type="button" className="absolute w-8 top-0.5 right-0.5 p-0.5 bg-white shadow rounded-full bg-green-100" aria-label={stockProductData.name}>
