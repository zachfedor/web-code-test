...
const StockProductsDataMap = new Map(
  StockProductsData.map((product) => [product.itemNumber, product])
);

const BuyList = ({ items, removeFromBuyListHandler }) => (
  <div className="mt-4">
    ...
    <div className="border border-gray-200 p-4 rounded shadow mt-2 grid grid-cols-1 gap-y-4">
      {items.length > 0 && items.map((item) => {
        const product = StockProductsDataMap.get(item.itemNumber);
        return (
          <div className="grid border border-gray-200 gap-x-4 rounded shadow p-4" style={{ gridTemplateColumns: "1fr 10fr 1fr 0fr 1fr 0fr 1fr 0fr", }} key={item.itemNumber}>
            ...
            <div className="flex items-center justify-center">
              <input type="number" className="border border-gray-300 rounded w-24 text-lg px-2" value={item.quantity}/>
            </div>
            ...
            <div className="flex items-center justify-end">
              <button type="button" className="bg-red-100 hover:bg-red-200 p-1 rounded ml-2" aria-label="Remove" onClick={removeFromBuyListHandler(item.itemNumber)}>
